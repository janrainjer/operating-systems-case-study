...
export default function EditCommentPage({
  ...
  const saveReview = async () => {
    if (blog) {
      ...
      for (let i = 0; i < uploadImages.length; i++) {
        formData.append(`oldImages`, oldImage[i]);
      }
      ...
      const res = await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND}/review/${commentId}`,
        formData,
        {
          headers: {
  ...
  const fetchComment = async () => {
    const res = await axios.get<IComment>(
      `${process.env.NEXT_PUBLIC_BACKEND}/review/get-review-by-reviewid/${commentId}`
    );
    const blog = await axios.get(
      `${process.env.NEXT_PUBLIC_BACKEND}/blogs/brief/${res.data.blogId}`
    );
    const temp = {
      title: res.data.title.length,
      description: res.data.description.length,
      recommendActivity: res.data.recommendActivity.length,
      spendTime: res.data.spendTime.length,
    };
    setCount(temp);
    setOldImage(res.data.images)
    setComment(res.data);
    setBlog(blog.data);
  };
